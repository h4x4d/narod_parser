...
async def process_page(session, url, root, binary):
...
async def get_page(session: aiohttp.ClientSession, url: str, root=0):
    ...
    return await safe_run(process_page(session, url, root, binary), url)
